> ⚠️ **历史文档（已过时）**：本文档记录的是早期基于 MinerU（托管 `mineru-api` 子进程、需 Python/torch/模型下载）的技术方案。
> 项目后续已移除该方案，改为**纯本地 AnyDoc 引擎**（进程内、无 ML 模型、无 Python、无联网）。
> 当前实现请以 [`README.zh.md`](../README.zh.md)、[`开发调试指南.md`](./开发调试指南.md) 与源码为准。以下内容仅作决策沿革参考。
